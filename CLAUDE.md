...
- **v1 "Ask Index Builder" is retired (2026-07-29).** Its Gemini store `observer-municipal-docs` was deleted to free indexing quota — that quota ceiling was what blocked 20 public notices from indexing. `ask-sources.json` at the repo root and the `ask_documents` Data Table are orphaned leftovers; the builder `qZQg4E4vQPhwxmeH` is deactivated. `ask_rate_limits` and `ask_config` are still shared with the live system — do not delete those.
